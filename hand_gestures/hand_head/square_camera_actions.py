...
# Function to check if a point is inside a diagonal zone
def is_point_in_diagonal_zone(point, square_x, square_y, square_size, zone_number):
    x, y = point
    ...
    if 0 <= rel_x <= 1 and 0 <= rel_y <= 1:
        if zone_number == 0:  # Top zone (above both diagonals)
            return rel_y <= 0.5 and rel_y <= 1 - rel_x
        elif zone_number == 1:  # Right zone (right of both diagonals)
            return rel_x >= 0.5 and rel_y <= rel_x
...
while cap.isOpened():
    ...
    if hand_results.multi_hand_landmarks:
        ...
        for idx, hand_landmarks in enumerate(hand_results.multi_hand_landmarks):
            # Determine if it's left or right hand - KEEPING YOUR ASSIGNMENT LOGIC
        ...
        for face_landmarks in face_results.multi_face_landmarks:
            ...
            if control_enabled and left_hand_in_left_square and right_hand_in_right_square:
                if 'Left' in hand_landmarks_dict and 'Right' in hand_landmarks_dict:
                    ...
                    # Display fingers together status
                    fingers_status = "Fingers together: YES" if fingers_together else "Fingers together: NO"
                    cv2.putText(
                        image,
                        fingers_status,
                        (10, 150),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        1,
                        (0, 255, 255) if fingers_together else (0, 0, 255),
                        2
                    )
                    ...
                    if fingers_together:
                        ...
                        if left_zone_square and 'Left' in hand_landmarks_dict:
                            index_tip = hand_landmarks_dict['Left'].landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP]
                            ih, iw, _ = image.shape
                            ...
                            for i in range(4):
                                if is_point_in_diagonal_zone(index_tip_pos, sq_x, sq_y, square_size, i):
                                    current_zone = f"Zone {i+1}"
                                    # Print zone to console
                                    print(f"Index finger tip in: {current_zone}")
                                    
                                    # Also display it on the image
                                    cv2.putText(
                                        image, 
                                        f"Detected in: {current_zone}",
                                        (10, 190), 
                                        cv2.FONT_HERSHEY_SIMPLEX, 
                                        1, 
                                        (0, 255, 255), 
