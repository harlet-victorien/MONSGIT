...
while cap.isOpened():
    ...
    if face_results.multi_face_landmarks:
        for face_landmarks in face_results.multi_face_landmarks:
            # Skip drawing full mesh - we'll only show specific points
            ...
            cv2.circle(image, face_center, 8, (255, 0, 0), -1)
            cv2.circle(image, face_center, 10, (0, 0, 255), 2)  # Add outline for better visibility
            cv2.putText(image, "Nose", (face_center[0] + 10, face_center[1]), 
    ...
    # Display
    cv2.imshow('Hand and Face Distance Tracking', image)
